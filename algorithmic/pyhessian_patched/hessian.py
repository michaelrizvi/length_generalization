...
class hessian():
    ...
    def density(self, iter=100, n_v=1):
        ...
        for k in range(n_v):
            ...
            # PATCHED: Use torch.linalg.eig instead of deprecated torch.eig for PyTorch 2.0+
            eigen_values, eigen_vectors = torch.linalg.eig(T)
            # Extract real parts (Hessian is symmetric, so eigenvalues should be real)
            eigen_list = eigen_values.real
            weight_list = (eigen_vectors[0, :].real)**2
